...
def index_of_max(digits: list[int]) -> int:
    ...
    for i in range(1, len(digits)):
        if digits[i] > digits[max_i]:
            max_i = i
    # print(f'Picking {digits[max_i]} (index {max_i}) out of {digits}')
    return max_i

def get_fancy_line_joltage(digits: list[int], num_digits_needed: int = 12) -> int:
    # Find the first largest digit within digits[:-num_digits_needed]
    ...
    # print(f'get_fancy_line_joltage digits:{digits} num_digits_needed:{num_digits_needed}')
...
# for calculator in (('Part Two', get_fancy_line_joltage),):
    print(calculator[0])
